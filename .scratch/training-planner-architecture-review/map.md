...
- [State change and side-effect consistency](issues/03-state-change-and-side-effect-consistency.md) — no code changes; everything audited (the form component's `effect()`/`subscribe()` split, the dashboard's guard-then-branch repetition, the deferred RxJS-pipeline guard rewrite, ad hoc `notify()` calls) judged fine as-is. Only outcome: a new "Reactive side effects" convention (signals → `effect()`/`computed()`, stream-native logic → RxJS, `untracked()` needs explicit sign-off) written to `CONTEXT.md`'s Architecture conventions.
- [Single-file-component convention](issues/04-single-file-component-convention.md) — templates ≤100 lines inline as a bare template-literal string (styles stay split, for now); every current `program` component qualifies, including `program-dashboard.component.ts`. Written as [ADR-0008](../../docs/adr/0008-inline-templates-for-small-components.md); `nx g component` now defaults to `inlineTemplate: true`. Retrofit spec: [Retrofit program components to single-file](issues/05-retrofit-program-components-to-single-file.md).
- [Retrofit program components to single-file](issues/05-retrofit-program-components-to-single-file.md) — landed: all 10 remaining split-template `program` components inlined (templates only, styles untouched), sibling `.html` files deleted. Lint clean, production build green, tests green; manual browser pass flagged for the user (no auth-emulator path).
...
- Whether stylesheets should eventually inline too, like templates now do (ADR-0008) — deliberately deferred rather than ruled out; revisit once there's a concrete reason to.
